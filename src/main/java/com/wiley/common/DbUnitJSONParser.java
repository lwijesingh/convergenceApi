...
public class DbUnitJSONParser extends AbstractDataSet {
         ...
         */
        public DbUnitJSONParser(File file) {
         ...
         */
        public DbUnitJSONParser(InputStream is) {
        ...
        private class JSONITableParser {
             ...
             */
            public ITableMetaData getMetaData(String tableName, List<Map<String, Object>> entry) {
                ...
                for (Map<String, Object> row : entry) {
                    for (Map.Entry<String, Object> column : row.entrySet()) {
                            List<Map<String, String>> col =(List<Map<String, String>>) column.getValue();
                            for (int i=0;i<col.size();i++) {
                                columns.add(col.get(i).get("name").toString());
                            }
                    }
                }
            ...
            private void fillRow(DefaultTable table, Map<String, Object> row, int rowIndex) {
                try {
                    ...
                    for (Map.Entry<String, Object> column : row.entrySet()) {
                        List<Map<String, String>> col =(List<Map<String, String>>) column.getValue();
                        for (int i=0;i<col.size();i++) {
                            table.setValue(rowIndex, col.get(i).get("name"), col.get(i).get("value"));
                        }
                    }
                } catch (Exception e) {
                    throw new RuntimeException(e.getMessage(), e);
